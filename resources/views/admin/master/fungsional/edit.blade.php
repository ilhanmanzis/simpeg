    ...
    <div class="p-8">
        ...
        <div class=" mx-5 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                <form action="{{ route('admin.fungsional.update', ['id' => $fungsional['id_fungsional']]) }}"
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ $fungsional['nama_jabatan'] }}"
                        ...
                        <div>
                            ...
                            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                <select name="golongan" required
                                    class="dark:bg-dark-900 shadow-theme-xs  focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border  bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden  dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 {{ $errors->has('golongan') ? 'border-error-300 focus:border-error-300 dark:border-error-700 dark:focus:border-error-800' : 'border-gray-300 focus:border-brand-300 dark:border-gray-700' }}">
                                    <option value="">Pilih Golongan</option>
                                    @foreach ($golongans as $golongan)
                                        <option value="{{ $golongan['id_golongan'] }}"
                                            {{ $fungsional->golongan->id_golongan == $golongan['id_golongan'] ? 'selected' : '' }}>
