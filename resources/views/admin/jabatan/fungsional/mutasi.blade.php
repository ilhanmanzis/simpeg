            ...
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                <form action="{{ route('admin.jabatan.fungsional.mutasi.store', ['id' => $user['id_user']]) }}"
                    method="post" enctype="multipart/form-data" x-data="{
                        // ID golongan saat ini dari server (angka urut: 1=IA, dst)
                        currentGolonganId: Number(`{{ $golongan->golongan->id_golongan ?? 0 }}`),
                        // Minimal dari pilihan fungsional
                        golonganMinimalId: 0,
                        golonganMinimalName: '',
                        // TAMBAHAN: state loading untuk modal
                        loading: false,
                        ...
                        updateGolongan(event) {
                            ...
                            const selected = this.fungsionals.find(f => f.id_fungsional == selectedId);
                        ...
                        handleSubmit(e) {
                            // validasi syarat golongan minimal
                            if (this.golonganMinimalId > 0) {
                                if (Number(this.currentGolonganId) < Number(this.golonganMinimalId)) {
                                    alert(`Golongan saat ini belum memenuhi syarat, minimal (${this.golonganMinimalName}).`);
                                    return; // batalkan submit
                                }
                            }
                            // LOLOS: tampilkan modal loading lalu submit
                            this.loading = true;
                            e.target.submit();
                        }
                    }"
