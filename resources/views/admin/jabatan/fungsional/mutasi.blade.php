<x-layout>
    ...
    <div class="p-8">
        <div x-data="{
            // ====== STATE DARI SERVER ======
            currentGolonganId: Number(`{{ $golongan->golongan->id_golongan ?? 0 }}`),
            hasGolongan: {{ $golongan ? 'true' : 'false' }},
            fungsionals: @js($fungsionals),
        
            // ====== STATE UI ======
            golonganMinimalId: 0,
            golonganMinimalName: '',
            loading: false,
            showRequireGolonganModal: {{ $golongan ? 'false' : 'true' }},
            showWarnModal: false,
            warnTitle: 'Syarat Golongan Belum Terpenuhi',
            warnBody: '',
        
            updateGolongan(event) {
                const selectedId = event.target.value;
                const selected = this.fungsionals.find(f => String(f.id_fungsional) === String(selectedId));
                if (selected && selected.golongan) {
                    this.golonganMinimalId = Number(selected.golongan.id_golongan || 0);
                    this.golonganMinimalName = String(selected.golongan.nama_golongan || '');
                } else {
                    this.golonganMinimalId = 0;
                    this.golonganMinimalName = '';
                }
            },
        
            handleSubmit(e) {
                // jika belum punya golongan: tampilkan modal wajib isi golongan
                if (!this.hasGolongan) {
                    this.showRequireGolonganModal = true;
                    return;
                }
        
                // validasi minimal golongan -> show modal cantik
                if (this.golonganMinimalId > 0 && Number(this.currentGolonganId) < Number(this.golonganMinimalId)) {
                    this.warnBody = `Golongan Anda saat ini belum memenuhi syarat minimal (${this.golonganMinimalName}). Silakan tingkatkan golongan terlebih dahulu.`;
            ...
            <div class="mx-5 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                    <form action="{{ route('admin.jabatan.fungsional.mutasi.store', ['id' => $user['id_user']]) }}"
                        method="post" enctype="multipart/form-data" @submit.prevent="handleSubmit($event)">

                        @csrf

                        <!-- Kunci semua field jika belum punya golongan -->
                        <fieldset :disabled="!hasGolongan">
                            <div class="grid grid-cols-1 gap-1 sm:grid-cols-1">
                                @if ($dosen || $golongan)
                                    <div class="mt-2 grid sm:grid-cols-2 gap-3 text-sm">
                                        ...
                                        <div class="flex items-center">
                                            <h3 class="text-base font-medium text-gray-800 dark:text-white/90 mb-5">
                                                Golongan Saat Ini :
                                                {{ $golongan?->golongan?->nama_golongan ?? 'Belum ada' }}
                                ...
                                <div class="flex justify-between mb-2">
                                    <div class="w-1/2">
                                        <label
                                            ...
                                            Jabatan Fungsional<span class="text-error-500">*</span>
                                        </label>
                                        <div class="relative z-20 bg-transparent">
                                            <select name="fungsional"
                                                class="dark:bg-dark-900 shadow-theme-xs focus:ring-brand-500/10 h-11 w-full appearance-none rounded-lg border bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 {{ $errors->has('fungsional') ? 'border-error-300 focus:border-error-300 dark:border-error-700 dark:focus:border-error-800' : 'border-gray-300 focus:border-brand-300 dark:border-gray-700' }}"
                                                @change="updateGolongan($event)" required>
                                                <option value="">Pilih Jabatan Fungsional</option>
                                                @foreach ($fungsionals as $fungsional)
                                                    <option value="{{ $fungsional['id_fungsional'] }}"
                                                        {{ old('fungsional') == $fungsional['id_fungsional'] ? 'selected' : '' }}
                                                        class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                                        {{ $fungsional['nama_jabatan'] }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            <span
                                    ...
                                    <div class="w-1/2 ml-3">
                                        ...
                                        <input type="text" name="name" x-model="golonganMinimalName"
                                            class="bg-gray-200 shadow-theme-xs focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 {{ $errors->has('name') ? 'border-error-300 focus:border-error-300 dark:border-error-700 dark:focus:border-error-800' : 'border-gray-300 focus:border-brand-300 dark:border-gray-700' }}"
                                            readonly disabled />
                                        <p class="mt-1 text-xs text-gray-500 dark:text-white/50">Diatur otomatis sesuai
                                            jabatan.</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex justify-between mb-2">
                                    ...
                                    <div class="w-1/3">
                                        <label
                                            class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                            Angka Kredit<span class="text-error-500">*</span>
                                        </label>
                                        <input type="text" name="angka_kredit" value="{{ old('angka_kredit') }}"
                                            class="dark:bg-dark-900 shadow-theme-xs focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 {{ $errors->has('angka_kredit') ? 'border-error-300 focus:border-error-300 dark:border-error-700 dark:focus:border-error-800' : 'border-gray-300 focus:border-brand-300 dark:border-gray-700' }}"
                                            required />
                                        @error('angka_kredit')
                                            <p class="text-theme-xs text-error-500 my-1.5">{{ $message }}</p>
                                        @enderror
                        ...
                        <button type="submit"
                            ...
                            <span
                                x-text="loading ? 'Menyimpan…' : (hasGolongan ? 'Simpan' : 'Lengkapi Golongan Dulu')"></span>
                        </button>

                        <!-- Modal: WAJIB ISI GOLONGAN -->
                        <div x-show="showRequireGolonganModal" x-cloak
                            ...
                            <div role="dialog" aria-modal="true"
                                ...
                                <div class="flex items-start gap-3">
                                    <svg class="h-6 w-6 text-rose-600 dark:text-rose-400 mt-0.5"
                                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        ...
                        <!-- Modal: PERINGATAN MINIMAL GOLONGAN -->
                        <div x-show="showWarnModal" x-cloak
                            class="fixed inset-0 z-[1000] flex items-center justify-center bg-black/40">
                            <div role="dialog" aria-modal="true"
                                class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
                                <div class="flex items-start gap-3">
                                    <svg class="h-6 w-6 text-amber-500 dark:text-amber-400 mt-0.5"
                                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                        <path stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                            stroke-linejoin="round" d="M12 8v4m0 4h.01M12 2l9 18H3L12 2z" />
                                    </svg>
                                    <div>
                                        <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90"
                                            x-text="warnTitle"></h3>
                                        <p class="mt-1 text-sm text-gray-600 dark:text-white/70" x-text="warnBody">
                                        </p>
                                        <div class="mt-4 flex items-center gap-2">
                                            <button type="button" @click="showWarnModal=false"
                        ...
                        <!-- Modal Loading (sudah ada di versi kamu) -->
                                ...
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Mohon
                                            tunggu…</h3>
                                        <p class="mt-1 text-sm text-gray-600 dark:text-white/70">
                                            Sedang memproses pengajuan dan/atau mengunggah berkas. Jangan menutup atau
