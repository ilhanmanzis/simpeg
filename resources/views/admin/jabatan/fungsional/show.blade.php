                        ...
                        <div class="p-5 border border-gray-100 dark:border-gray-800 sm:p-6 mt-5">
                            <div
                                ...
                                <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90  -mt-5 ">
                                    Jabatan Fungsional {{ $user->dataDiri->name }} Saat Ini</h2>

                                <a href="{{ route('admin.jabatan.fungsional.mutasi', ['id' => $user->id_user]) }}"
                                ...
                                <div class="w-full">
                                    ...
                                    <div class="w-full ">
                                        ...
                                        <div class="flex">
                                            ...
                                            <div class="w-4">:</div>
                                            <div class="flex-1">{{ $dosen->status ?? '-' }}</div>
                                        </div>
                                        <div class="flex">
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="px-5 py-4 sm:px-6">
                                                        <div class="flex items-center">
                                                            <div class="flex -space-x-2">
                                                                <button type="button"
                                                                    @click="confirmDelete('{{ route('admin.jabatan.fungsional.mutasi.delete', ['id' => $riwayat['id_fungsional_user']]) }}', '{{ strtolower($riwayat->status ?? '') }}', '{{ $riwayat->fungsional->nama_jabatan }}')"
        ...
        <script>
            ...
            document.addEventListener('alpine:init', () => {
                Alpine.data('riwayatDelete', () => ({
                    ...
                    confirmDelete(url, status, jabatan) {
                        this.actionUrl = url;

                        const isAktif = (status || '').trim() === 'aktif';
                        this.message = isAktif ?
                            `Anda akan menghapus riwayat jabatan fungsional "${jabatan}" yang masih AKTIF. Jika Anda melanjutkan, jabatan fungsional saat ini akan menjadi KOSONG sampai Anda menetapkan yang baru. Lanjutkan?` :
                            `Apakah Anda yakin ingin menghapus riwayat jabatan fungsional "${jabatan}"?`;

                        this.show = true;
                    },

                    proceed() {
                        // buat form POST dinamis dengan _method=DELETE
