        ...
        <div x-data="{ pageName: `{{ $title }}` }">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-3 mx-5">
                <x-breadcrumb :items="[
                    ...
                    $user->dataDiri->name => route('admin.jabatan.golongan.show', $user->id_user),
                    'Mutasi Golongan' => '#',
                ]" />
            </div>
        ...
        <div class=" mx-5 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex justify-between">
                <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
                    Mutasi Golongan {{ $user->dataDiri->name }}
                </h3>
            </div>
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                <form action="{{ route('admin.jabatan.golongan.mutasi.store', ['id' => $user['id_user']]) }}"
                    method="post" enctype="multipart/form-data" x-data="{ loading: false }"
                    @submit.prevent=" if ($event.target.checkValidity()) {
                        loading = true;           
                        $event.target.submit();   
                    } else {
                        ...
                        <div class="flex justify-between mb-2">
                            <div class="w-1/3">
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Golongan<span class="text-error-500">*</span>
                                </label>
                                    ...
                                    <select name="golongan"
                                        ...
                                        @foreach ($golongans as $golongan)
                                            <option value="{{ $golongan['id_golongan'] }}"
                                                {{ old('golongan') == $golongan['id_golongan'] ? 'selected' : '' }}
