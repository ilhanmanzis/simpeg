    ...
    <div class="p-8">
        ...
        <div x-data="{ pageName: `{{ $title }}` }">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-3 mx-5">
                <x-breadcrumb :items="[
                    'Pengajuan Jabatan Fungsional' => route('admin.pengajuan.fungsional'),
                    'Lihat' => '#',
                ]" />
            </div>
            ...
            <div class=" m-5 border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div class="px-5 py-4 sm:px-6 sm:py-5 flex justify-between">
                    <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
                        {{ $title }} {{ $pengajuan->user->nama_lengkap }}
                    </h3>
                </div>
                <div
                    ...
                    <div class="w-full">
                        ...
                        <div class="w-full ">
                            <div class="flex">
                                ...
                                <div class="w-4">:</div>
                                <div class="flex-1">
                                    {{ $pengajuan->fungsional->nama_jabatan }}
                                </div>
                            </div>
                            <div class="flex">
                                <div class="w-32 font-semibold">Tanggal Mulai</div>
                                <div class="w-4">:</div>
                                <div class="flex-1">{{ $pengajuan->tanggal_mulai }}
                                </div>
                            </div>
                            <div class="flex">
                                <div class="w-32 font-semibold">Tanggal Selesai</div>
                                <div class="w-4">:</div>
                                <div class="flex-1">{{ $pengajuan->tanggal_selesai ?? '-' }}
                                </div>
                            </div>
                            <div class="flex">
                                <div class="w-32 font-semibold">Angka Kredit</div>
                                <div class="w-4">:</div>
                                <div class="flex-1">{{ $pengajuan->angka_kredit ?? '-' }}
                                </div>
                            </div>
                            <div class="flex">
                                ...
                                <div class="flex-1">
                                    @if ($pengajuan->sk)
                                        <a href="{{ route('file.sk', $pengajuan->sk) }}" target="_blank"
                            ...
                            @if ($pengajuan->status == 'pending')
                                ...
                                <div class="flex justify-between w-full">
                                        ...
                                        <div x-show="openTolak"
                                            ...
                                            <div @click.away="openTolak = false"
                                                ...
                                                <h2 class="text-lg font-bold mb-4 text-gray-800 dark:text-white/90">
                                                    ...
                                                    ingin menolak?</h2>
                                                <form method="POST"
                                                    action="{{ route('admin.pengajuan.fungsional.tolak', ['id' => $pengajuan->id_pengajuan_fungsional]) }}">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="mb-4">
                                    ...
                                    <!-- SETUJU FUNGSIONAL + LOADING MODAL -->
                                    <div x-data="{ loadingFungsional: false }" class="w-1/4">
                                        <form
                                            action="{{ route('admin.pengajuan.fungsional.setuju', ['id' => $pengajuan['id_pengajuan_fungsional']]) }}"
                                            method="post" @submit="loadingFungsional = true">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit"
                                                ...
                                                :disabled="loadingFungsional">
                                                <svg x-show="loadingFungsional"
                                                ...
                                                <span x-text="loadingFungsional ? 'Memproses…' : 'Setuju'"></span>
                                            </button>
                                        </form>

                                        <!-- Overlay modal -->
                                        <div x-show="loadingFungsional" x-cloak
